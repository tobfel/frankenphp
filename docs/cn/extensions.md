...
虽然第一点不言自明，但第二点可能更难理解。让我们在本指南的后面深入了解类型转换。
...
- **`my_extension_generated.go`** - 生成的文件，包含调用你的函数的 CGO 包装器
- **`my_extension.stub.php`** - 用于 IDE 自动补全的 PHP 存根文件
...
> [!IMPORTANT]
> **你的源文件（`my_extension.go`）从未被修改。** 生成器会创建一个单独的 `_generated.go` 文件，其中包含调用你的原始函数的 CGO 包装器。这意味着你可以安全地对源文件进行版本控制，而无需担心生成的代码污染它。
...
`AssociativeArray` 表示一个[哈希映射](https://en.wikipedia.org/wiki/Hash_table)，由 `Map: map[string]any` 字段和可选的 `Order: []string` 字段组成（与 PHP“关联数组”不同，Go map 不是有序的）。
...
- `frankenphp.PHPAssociativeArray(arr frankenphp.AssociativeArray) unsafe.Pointer` - 转换为具有键值对的有序 PHP 数组
- `frankenphp.PHPMap(arr map[string]any) unsafe.Pointer` - 将 map 转换为具有键值对的无序 PHP 数组
- `frankenphp.PHPPackedArray(slice []any) unsafe.Pointer` - 将 slice 转换为仅包含索引值的 PHP 打包数组
- `frankenphp.GoAssociativeArray(arr unsafe.Pointer, ordered bool) frankenphp.AssociativeArray` - 将 PHP 数组转换为有序 Go `AssociativeArray`（带顺序的 map）
- `frankenphp.GoMap(arr unsafe.Pointer) map[string]any` - 将 PHP 数组转换为无序 Go map
- `frankenphp.GoPackedArray(arr unsafe.Pointer) []any` - 将 PHP 数组转换为 Go slice
- `frankenphp.IsPacked(zval *C.zend_array) bool` - 检查 PHP 数组是否为打包（仅索引）或关联（键值对）
